...
* Класс Parser: парсит строку на команду и аргументы
    ...
    * Метод getNext - отрезает очередную слева часть запроса по пайпу, если это еще возможно.
      ...
      Парные кавычки удаляются везде, кроме присваивания (описана ниже). Все, что находилось внутри кавычек, не делится на аргументы.
      Примеры: "echo" x -> x; cat READ'ME.md' -> выведет содержимое README.md; x=" spaces  " -> сохранит значение с пробелами и кавычками.
      Если встречен знак '=' (не находящийся в кавычках и перед ним нет пробела), то кавычки не удалются. То, что стоит до него, будет переменной, а то, что после до пробела - значением.
