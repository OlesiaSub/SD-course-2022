...
* Класс CommandGrep
    * Выбор библиотеки для разбора параметров
      
      Сравнение:
      * JCommander: связывает параметры командной строки с полями в классе, 
      позволяет контролировать более тонкие настройки параметров (обязательность параметра, количество дополнительных аргументов),
      есть возможность создания сложных типов параметров, настройки многократного использования параметра, создание кастомных проверок параметра.
      * Commons.CLI: каждый параметр описывается как объект Option, позволяет указать полное и краткое имя параметра, наличие дополнительных аргументов и описание. 
      Можно создавать и более сложные параметры с кастомными свойствами (например указать количество аргументов и их тип).
      
      В итоге была выбрана библиотека Commons.CLI так как она показалась более простой для использования. JCommander позволяет реализовывать разбор более сложных параметров,
      но для реализации наших параметров такой необходимости нет.
    * Валидация аргументов:
      * Динамические: ожидается ноль или один аргумент - строка. Воспринимается как содержимое файла.
      * С помощью DefaultParser из статических аргументов строится строка запуска
      * Статические: валидируются аргументы строки запуска (паттерн и название файла). 
      Если переданного файла не существует, то возвращаем код возврата, соответствующий ошибке некорректности аргументов.
      Если аргументов < 2 и динамических аргументов нет или аргументов < 1, 
      то возвращаем код возврата, соответствующий ошибке некорректности аргументов.
    * execute:
        * Если среди статических аргументов есть название файла - динамические игнорируются
        * Иначе в качестве файла используется динамический аргумент
        * Ключи разбираются с помощью библиотеки Commons.CLI. Неподдерживаемые ключи игнорируются.
    * Обработка параметров:
        * Параметры обрабатываются независимо друг от друга
        * -w: Поиск слова только целиком. Реализуется с помощью изменения паттерна на `\bPATTERN\b`
        * -i: Регистронезависимый поиск. Реализуется с помощью добавления ключа `Pattern.CASE_INSENSITIVE` при компиляции паттерна
        * -A NUM: Печать еще NUM строк, после строки с найденным паттерном. Реализация предполагает пересечение найденных областей печати.
      Для этого заведен счетчик, в случае, если паттерн нашелся в строке, то счетчику присваивается значение NUM.
      Далее печатаются строки, пока счетчик > 0, либо пока в очередной строке не найдется паттерн.
