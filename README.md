...
* Класс Parser: парсит строку на команду и аргументы
  ...
  * Метод getNext - отрезает очередную слева часть запроса по пайпу, если это еще возможно.
  ...
  Все, что идет от открывающей кавычки до закрывающей (включая пробелы) - 
  один аргумент в случае, если открывающая кавычка идет после пробела или знака =.
  В противном случае (типа cat README.md'README.md') кавычки считаются просто символами и конкатенируются с тем, что стоит до них.
  Возвращает массив строк, в котором первый элемент - название самой команды, остальные - ее аргументы.
